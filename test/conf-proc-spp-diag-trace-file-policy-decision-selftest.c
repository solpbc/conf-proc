...
static int test_compatibility(void)
{
    ...
    for (v = 0; v <= 0xffffu; v++) {
        if (v == SPP_DIAG_TRACE_PROVENANCE_EVENT_FILE_OPEN_ATTEMPT ||
            ...
            || v == SPP_DIAG_TRACE_PROVENANCE_EVENT_NETWORK_POLICY_DECISION
#endif
#ifdef SPP_DIAG_TRACE_PROVENANCE_EVENT_OPERATION_RETURN
            || v == SPP_DIAG_TRACE_PROVENANCE_EVENT_OPERATION_RETURN
#endif
#ifdef SPP_DIAG_TRACE_PROVENANCE_EVENT_TASK_EXIT
            || v == SPP_DIAG_TRACE_PROVENANCE_EVENT_TASK_EXIT
#endif
        ) {
            continue;
        }
